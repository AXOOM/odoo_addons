...
from ..tools import erp_event_decorator

_logger = logging.getLogger(__package__)


class ErpEventRule(models.Model):
    _name = 'erp.event.rule'
    _description = 'ERP Event Rule'
    ...
    def _add_action(self):
        if not self.action_id:
            vals = {
                'name': _('View ERP-Event logs'),
                'res_model': 'erp.event.log',
    ...
    @tools.cache()
    def _check_erp_event_rule(self, cr):
    ...
    def _register_hook(self, cr, ids=None):
        ...
        for rule in self.browse(cr, SUPERUSER_ID, ids):
            ...
            if not model_obj:
                continue
            if rule.active and not hasattr(model_obj, 'erp_event_rule'):
                for method in self._methods:
                    model_obj._patch_method(method, erp_event_decorator())
                model_obj.erp_event_rule = True
                updated = True
            if not rule.active and hasattr(model_obj, 'erp_event_rule'):
                for method_name in self._methods:
                    method = getattr(model_obj, method_name)
                    while hasattr(method, 'origin'):
                        if method.__name__ == 'erp_event_wrapper':
                            model_obj._revert_method(method_name)
                            break
                        method = method.origin
                del model_obj.erp_event_rule
                updated = True
        if updated:
            self.clear_caches()
        return updated
    ...
    def create(self, vals):
        vals['state'] = 'done'
        rule = super(ErpEventRule, self).create(vals)
        rule.update_rule()
    ...
    @api.multi
    def write(self, vals):
        res = super(ErpEventRule, self).write(vals)
        self.update_rule()
    ...
    def unlink(self):
        ...
        return super(ErpEventRule, self).unlink()
    ...
    def log(self, method, old_values=None, new_values=None, fields_to_read=None):
        ...
        if old_values or new_values:
            ...
            for res_id in data:
                self.env['erp.event.log'].sudo().create({
                    'user_id': self._uid,
