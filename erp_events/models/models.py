...
def _read_from_database(self, field_names, inherited_field_names=[]):
    ...
    if self._context.get('history_revision') and getattr(self._model, 'erp_event_rule', None):
        history_date = self._context.get('history_revision')
        create_rule = self.pool['erp.event.rule']._check_erp_event_rule(self._cr).get(self._name, {}).get('create')
        date_operator = create_rule and '>' or '>='
        domain = [('model', '=', self._name), ('res_id', 'in', self.ids),
                  ('create_date', date_operator, history_date)]
        logs = self.env['erp.event.log'].sudo().search(domain, order='create_date desc')
        for record in self:
            vals = {}
            for log in logs:
