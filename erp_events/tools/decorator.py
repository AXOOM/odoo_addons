...
def erp_event_decorator():
    def erp_event_wrapper(self, *args, **kwargs):
        origin = erp_event_wrapper.origin
        while hasattr(origin, 'origin'):
            origin = origin.origin
        method = origin.__name__
        records, fields_to_read = _get_args(self, method, args, kwargs)
        rule_obj = records.env['erp.event.rule']
        rule_id = rule_obj._model._check_erp_event_rule(records._cr).get(records._name, {}).get(method)
        if rule_id:
        ...
        result = erp_event_wrapper.origin(self, *args, **kwargs)
